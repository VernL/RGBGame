...
function initializeBoard(state) {
    ...
    state.targetRGB =  state.squaresColor[Math.floor((Math.random() * state.numSquares))].color
}
...
const mutations = {
    ...
    'CHECK_TILE' (state, payload){
        if( state.squaresColor[payload].color === state.targetRGB ) {
            ...
            state.win = true;
            for(let i = 0; i < state.numSquares; i++){
                state.squaresColor.splice(i, 1, {color : state.targetRGB, clicked : true})
            }
        } else {
            state.message = 'Wrong! Try Again!';
            state.squaresColor.splice(payload, 1, {color : '#232323', clicked : true})
        }
    },
    'GAME' (state) {
      state.game = true;
